...
iqtree <- read.tree('/Users/eilishmcmaster/Documents/LantCama/iqtree/LantCama_DLan23-8067_eilish_iqtree.nex.contree')

iqtree$node.label <- round(as.numeric(iqtree$node.label),0)

ggtree_obj <- ggtree(iqtree)
...
ggtree_obj <- ggtree(iqtree, size=0.3) %<+% x1 
...
hmt <- gheatmap(ggtree_obj, as.matrix(x1[,c('svdq_pop_label','morphid2','national2')]),
  ...
  # geom_label2(data=iqtree, aes(label=node),color='red', nudge_x = 0.0001, label.size=0, fill="transparent", size=0.5)+
  geom_label2(data=iqtree, aes(label=label, subset = !is.na(as.numeric(label)) & as.numeric(label) > 50),
...
plot_prob <- ggplot(data.frame(label=iqtree$node.label, x=1:length(iqtree$node.label), y=1:length(iqtree$node.label)),
...
ggsave("LantCama/outputs/LantCama_iqtree_maf2.pdf",
       combined_plot, width = 25, height = 40, units = "cm", dpi=600)

ggsave("LantCama/outputs/LantCama_iqtree_maf2.png",
...
ggsave("LantCama/outputs/LantCama_iqtree_maf2_collapsed.pdf",
       combined_plot, width = 15, height = 20, units = "cm", dpi=600)


ggsave("LantCama/outputs/LantCama_iqtree_maf2_collapsed.png",
       combined_plot, width = 15, height = 20, units = "cm", dpi=600)
