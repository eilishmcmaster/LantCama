...
mrbayes <- treeio::read.mrbayes('/Users/eilishmcmaster/Documents/LantCama/mrbayes/lantana eilish mrbayes full_2/infile.nex.con.tre')
...
hmt <- gheatmap(ggtree_obj, as.matrix(x1[,c('svdq_pop_label','morphid2','national2')]),
                ...
                custom_column_labels=c("SVDq cluster",'Morphid',"Country"), hjust=0) +
...
plot_prob <- ggplot(data.frame(label=mrbayes@data[["prob"]],
                               x=1:length(mrbayes@data[["prob"]] ), y=1:length(mrbayes@data[["prob"]] )),
...
nodes.identity <- c("7", "6", "5", "4", "3", "2", "1", "eawt")

names(nodes.identity) <- c(579, 561, 549,590, 746, 741, 717, 654)
nodes.to.collapse <- c(579, 561, 549,590, 746, 741, 717, 654)
...
scaleMyClade <- function(.p, .node) {
  ...
  if((scaling_factor*num_samples) <10){
    scaling_factor <- 10/num_samples
  }
...
collapseMyClade <- function(.p, .node) {
  ...
  .p <- .p + geom_cladelabel(node = .node,align = TRUE,label = label, vjust=-0.02,offset.text=-0.0005, 
                             fontsize = 2)
  return(.p)
}
...
hmt2 <- gheatmap(ggtree_obj2, as.matrix(x1[,c('svdq_pop_label','morphid2','national2')]),#'svdq_pop_label',
                 offset = 0.04, width = .07, font.size = 8,
                 colnames_angle = 90, colnames_position = "top",
                 custom_column_labels = c("SVDq cluster","morphid", "Country"), hjust = 0) +
  scale_fill_manual(values = c(svdq_pop_colours,nation_colours, morphid_colours), na.value = "white") +
  theme_tree2() +
...
combined_plot <- cowplot::plot_grid(hmt2, legends,nrow = 1, rel_widths = c(1, 0.2))


ggsave("LantCama/outputs/LantCama_mrbayes_maf2_collapsed.pdf",
       combined_plot, width = 15, height = 20, units = "cm", dpi=600)


ggsave("LantCama/outputs/LantCama_mrbayes_maf2_collapsed.png",
       combined_plot, width = 15, height = 20, units = "cm", dpi=600)
