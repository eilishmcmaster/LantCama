...
upgma$node.label <- as.numeric(upgma$node.label)
...
hmt <- gheatmap(ggtree_obj, as.matrix(x1[,c('svdq_pop','national2')]),
                offset=0.0005, width=.05, font.size=0,
  ...
  # geom_text2(data=upgma, aes(label=node), size=0.5, color="red")+
  geom_nodepoint(data=upgma, aes(color=as.numeric(label)), size=0.5)+scale_color_distiller(palette = "RdYlBu", direction=+1, na.value = NA)
